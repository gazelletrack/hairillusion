...
namespace :hair do 
  ...
  task :gen => :environment do
     ...
     orders = Order.where("id > 0 and parent_order_id is null and order_type ='recurrent' and next_delivery_date < '2016-10-01' and cancelled=false and hc_order=false").order("id asc").limit(25)
       ...
       unless order.next_delivery_date.nil?
        ...
        begin           
          #  total_price = 2495    
            ...
            puts "name: -#{customer_card.card_name}"
            puts "card_number -#{customer_card.card_number}"
            puts "ccv -#{customer_card.ccv}"
            puts "customer_card month: -#{customer_card.exp_month}"
            puts "customer_card exp_year -#{customer_card.exp_year}"
          ...
          order.next_delivery_date = Date.today+ 28.days  
          order.save
